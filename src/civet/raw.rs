...
pub struct ServerCallback<T> {
    callback: fn(&mut MgConnection, &T) -> Result<(), ()>,
    param: T
}

impl<T> ServerCallback<T> {
    pub fn new(callback: fn(&mut MgConnection, &T) -> Result<(), ()>, param: T) -> ServerCallback<T> {
...
fn raw_handler<T>(conn: *mut MgConnection, param: *c_void) -> int {
    ...
    task.run(|| {
        result = Some((callback.callback)(unsafe { &mut *conn }, &callback.param))
    });
    let _ = rx.recv();
